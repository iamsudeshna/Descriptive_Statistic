...
def read_dataset():
    global data_frame
    # Provide the full or relative path to the Excel file
    ...
    print("\n95% Confidence Interval for GOP_Year3:")
    print(confidence_interval)

    # mixed_type_column = data_frame.iloc[:, 12]
    # print(mixed_type_column.head())

    # data_frame['NIC_CODE'] = pd.to_numeric(data_frame['NIC_CODE'], errors='coerce')
...
def performance_measures():
    ...
    # Calculate 99% confidence intervals for the performance measures
    confidence_interval_GOP_Per_Employee = stats.norm.interval(0.99, loc=np.mean(data_frame['GOP_Per_Employee']),
                                                                   scale=stats.sem(data_frame['GOP_Per_Employee']))
    confidence_interval_Asset_Utilization_Ratio = stats.norm.interval(0.99, loc=np.mean(
    data_frame['Asset_Utilization_Ratio']),
                                                                          scale=stats.sem(
                                                                              data_frame['Asset_Utilization_Ratio']))
    ...
    print("\n1. ", measure1_desc)
    print("\n2. ", measure2_desc)
    # Calculate confidence intervals for both performance measures
...
def proportion_of_sssbe_in_population():
    ...
    if proportion_sssbe < 0.25:
        ...
        print("\nNo need for special incentives based on the proportion.")
...
def sssbe_and_ssi():
    ...
    # Calculate the proportion of better performing units for both groups based on measure 1
    ...
    # Perform a hypothesis test (e.g., chi-squared test) to compare proportions for measure 1
    # Define better performing units based on measure 1
